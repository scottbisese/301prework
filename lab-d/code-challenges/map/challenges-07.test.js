...
const forEachTwoToThe = arr => {
  let bathSalts = [];
  arr.forEach(num => bathSalts.push(Math.pow(2, num)));
  return bathSalts;
};
...
const evenOdd = arr =>
  arr.map(n => (isNaN(n) ? "N/A" : n % 2 ? "odd" : "even"));
...
const extractAbilities = arr => arr.map(pokedex => pokedex.ability.name);
...
const extractStats = arr => arr.map(pokedex => ({
  name: pokedex.stat.name,
  total: pokedex.effort + pokedex.baseStat
}));
