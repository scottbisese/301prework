...
const divisibleByFiveTwoToThePower = (input) => input.map(arr => arr.filter(val => typeof val === 'number' && val % 5 === 0).map(val => Math.pow(2, val)));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
let findMaleAndFemale = (data) => data.filter(char => char.gender === 'male' || char.gender === 'female').reduce((joined, current) => !joined ? current.name : `${joined} and ${current.name}`, '');
...
let findShortest = (data) => data.reduce((a, b) => !a || Number(b.height) < Number(a.height) ? b : a, null).name;
...
describe('Testing challenge 3', () => {
  ...
  test('It should return an empty array if the vals are not numbers', () => {
