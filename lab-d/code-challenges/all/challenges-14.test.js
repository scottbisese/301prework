...
const toTitleCase = (arr) => arr.map(str => str.slice(0, 1).toUpperCase() + str.slice(1));
...
let biggerThanLuke = (arr) => {
  const skywalkerMass = Number(arr.reduce((skywalker, current) => current.name === 'Luke Skywalker' ? current : skywalker, { mass: 77 }).mass);
  return arr.filter((character) => Number(character.mass) > skywalkerMass).map((character) => character.name).join(' - ');
};
...
const sortBy = (property, arr) => //ran out of time
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => url.match(/^https:\/\//); //we did this in book-app minus the escaping the //
