...
arr.map(number => number.substring(1,4) + number.substring(6,9) + number.substring(10,14));
...
const onlyOddChars = (str) => str.split('').filter((character, i) => i % 2 === 1 ? true : false).join('');
...
const unenrollBrook = (arr) => arr.map(array => array.filter(name => !name.includes('Brook')));
