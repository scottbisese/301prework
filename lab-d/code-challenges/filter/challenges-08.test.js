...
const oddValues = arr => arr.filter(n => n % 2 != 0);
...
const filterStringsWithVowels = arr => arr.filter(str => str.match(/[aeiou]/i));
...
const getBaseStatGreaterThan = (arr, minBaseStat) =>
  arr.filter(pokedex => pokedex.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) =>
  getBaseStatGreaterThan(arr, minBaseStat).map(pokedex => pokedex.stat.name);
