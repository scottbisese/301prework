...
const listFoods = (recipe) => {
  ...
  for (const ingredient of recipe.ingredients) {
    let brevity = ingredient.slice(ingredient.indexOf(' ') + 1);
    result.push (brevity.slice(brevity.indexOf(' ') + 1 ) );
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  ingredients.forEach(e => result.push(e.split(' ').slice(2, e.split(' ').length).join(' ')));
  return result;
};
...
const stepActions = (recipe) => {
  ...
  steps.forEach(e => result.push(e.split(' ')[0]));
  return result;
};
